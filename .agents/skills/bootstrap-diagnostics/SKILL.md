...
- `SECONDMATE_HANDOFF: secondmate <id>: pending card link(s): <n>` - that many item/card pairs in `state/handoff-cards/<id>` are still owed their Fleet Dashboard link, so those cards are still reading `not_started` while the work is already with the secondmate.
  Session start already retried them; a count that persists across runs means the board is unreachable, refusing the write, does not have the named card, or holds a pair that was superseded by a later `--card` for the same item, so run `bin/fm-backlog-handoff.sh --resume-pending` by hand and read its stderr for the per-pair reason before editing the record.
  Only the first three clear once the board or the card id is fixed; a superseded pair is deliberately never linked, so it never retires itself and holds the count non-zero indefinitely - resolve it through the `--card` revival and hand-check that [`docs/dashboard.md`](../../../docs/dashboard.md) owns, not by chasing a board fault that was never the reason.
