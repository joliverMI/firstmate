...
- `needs-review` - the next step is his and it is one word: the fleet has a recommended action it intends to take and is asking him to approve it before acting. `--plan` is REQUIRED and carries that recommendation: the card renders it in its own box with the approve button inside that same box, so what he approves is unmistakably the text he is looking at. The server refuses this status with no plan on both the create and the status path, because an approval box with nothing in it would record his consent to nothing at all. Write the plan as the action you will take, short enough to read on a phone and specific enough that approving it means something - "swap to the other supplier and re-run the fit check", not "proceed as discussed". A plan is not an ask, so the report-shaped-phrase guard deliberately does not apply to it; whether a plan is real and specific is the auditor's judgment, exactly as it is for a `needs-action` reason. Alone among the statuses a landing would otherwise advance, `bin/fm-teardown.sh` will NOT advance this card when its serving work lands unless his approval covers the plan the card displays: the approve button exists only here, so a card he has not answered stays here rather than disappearing into `review` where he could no longer answer it. **Expect the card to leave this status the moment he approves**: his approval moves it to `not_started` by itself and wakes firstmate, so a card still sitting in `needs-review` has NOT been approved, and moving an approved card out of here is not a step anyone owes (see "What he approves, and what an approval is worth" below). If the approved wording later needs correcting, `plan <id>` records the correction but does not bring the card back here; re-ask with `status <id> needs-review --plan <new text>` so the button is in front of him again.
...
The approve button records his consent.
...
What it does do, mechanically, is finish answering the question rather than leaving that to someone remembering: a current approval moves the card out of `needs-review` to `not_started` with the note "approved by the Admiral; awaiting dispatch", and wakes firstmate once with a durable `check: dashboard-approval <card-id>` record.
Handle that wake as you would any other `check:` wake - read the card, and dispatch the approved work at whatever rigor that project's delivery mode requires.
Dispatching is still yours; the approval never merges, deploys, or starts anything by itself.
A stale approval, or an approval on a card that has already moved on, moves nothing and wakes nobody.
[`docs/dashboard.md`](../../../docs/dashboard.md) "What an approval does mechanically" owns the full contract, including what happens when the wake cannot be published.
...
If the plan is edited afterwards, the approval is NOT carried over: the record of his word survives, but the card, `show`, and `--json` all report it as covering the old wording only and show both texts.
A plan edit alone does not ask him again, because an approved card has already left `needs-review` and the approve button exists only there; to re-ask, move the card back with `status <id> needs-review --plan <new text>`.
...
That command only ever corrects a plan, and the server enforces it: writing the FIRST plan onto a card that has never been needs-review is refused, because the move to needs-review is what puts the approval box in front of him.
Write the first plan with `status <id> needs-review --plan "<text>"`, and correct it with `plan` afterwards - including on a card that has since moved on, whose plan and approval deliberately outlive the status.
