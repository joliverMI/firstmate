...
That no-fetch path is a purely local fast-forward of tracked files, never a remote fetch, and it never touches the gitignored operational dirs, so a secondmate's backlog, projects, and in-flight work are never disturbed; a linked worktree advances immediately, while a standalone clone that lacks the target receives firstmate updates through `/updatefirstmate`'s fetch-based refresh.
...
`/updatefirstmate` is the one path that still follows that copy: it first updates the remote code root from its own configured upstream, then syncs the home to that refreshed code-root commit.
...
`stuck-crewmate-recovery`'s remote-secondmate note owns why an `unknown-remote` read or an unreachable-host send failure is never proof that the endpoint is dead or that the steer was lost.
