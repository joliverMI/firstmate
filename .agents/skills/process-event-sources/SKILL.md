...
`bin/fm-procevent.sh --help`, `bin/fm-procevent-lavish.sh --help`, `bin/fm-procevent-when.sh --help`, `bin/fm-procevent-quota.sh --help`, `bin/fm-procevent-river.sh --help`, and `bin/fm-procevent-remote-reply.sh --help` own the exact commands and flags.
...
: **When handling means running one downstream command whose effect must not repeat - forwarding captured feedback to a task through `fm-send.sh` is the motivating case - run it through `bin/fm-procevent.sh deliver <source-id> <sequence> -- <command>...` instead of running the command directly and separately remembering `handled` above.** Forgetting that second step across a restart, a compaction, or a turn that never got there is what let a repeated re-announcement of one unhandled generation repeat a forward for real; `deliver` runs the command and records handling as one call under one lock, so that gap cannot reopen. [`docs/configuration.md`](../../../docs/configuration.md#process-to-event-sources-stateprocevent) owns the exact guarantee and `bin/fm-procevent.sh`'s own header owns its usage.
...
- `deliver` runs its command at most once per generation - a repeat call for a generation it already delivered runs the command zero times, and a command that exits nonzero leaves the generation unhandled and eligible for the next retry rather than being marked delivered;
