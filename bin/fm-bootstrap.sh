...
#          fleet_sync) and the local continuity-deadman start, while still
...
if [ "${FM_BOOTSTRAP_DETECT_ONLY:-0}" != 1 ]; then
  ...
  # Clone refresh started above, overlapping the secondmate sweeps; join it here
  # so its output still lands in the same place in the digest as before.
