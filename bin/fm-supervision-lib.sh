...
# (state/x-watch.check.sh), or a trust-registered custom check
# (state/<id>.check-trust bound to state/<id>.check.sh via
# bin/fm-check-register.sh), and whether its watcher has a fresh liveness
...
#   FM_SUP_CHECKS         count of trust-registered custom checks
#   FM_SUP_NEEDED         true/false - in-flight work, an X-mode relay poll, a
#                         registered event source (a source is a wait on an
#                         external process, not a task, so it has no metadata),
#                         or a trust-registered custom check (likewise no task
#                         metadata of its own)
...
fm_supervision_status() {
  local state=$1 grace=${2:-${FM_GUARD_GRACE:-300}} meta source trust id beat m age
  FM_SUP_IN_FLIGHT=0
  FM_SUP_NEEDED=false
