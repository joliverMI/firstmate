...
\`OWNER_REPO=\$("$FM_ROOT/bin/fm-pr-destination-guard.sh" . --print-destination) && gh-axi pr create --repo "\$OWNER_REPO"\`
Append your own \`--title\` and \`--body\` arguments to that same command line; do not split it up. That guard mode reads this repo's own \`origin\` remote and prints \`owner/repo\` and nothing else, makes no network call, and exits non-zero with a diagnostic rather than printing a guess - so if it fails, nothing is created: append \`blocked: {its exact error}\` to the status file and stop. Then append \`done: PR {url}\` to the status file and stop, after confirming the returned URL's owner/repo matches the destination that command printed.
