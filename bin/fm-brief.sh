...
First write the PR body to a file, with a quoted heredoc delimiter so the shell expands and executes nothing inside it - a markdown body naming commands or paths in backticks would otherwise be run before \`gh-axi\` ever saw it, and its output substituted into the body:
