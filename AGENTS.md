...
config/pi-supervision-branch  Pi-only explicit project autonomy grants for the in-process supervision branch on a Pi primary; LOCAL, gitignored, not inherited; one exact project=<task-metadata project> per approved project, with unscoped/mixed/unlisted wakes preserving today's wake-to-main behavior; see docs/configuration.md "Pi supervision branch" and docs/pi-supervision-branch.md
...
config/wedge-alarm  optional active-alert channel directives shared by the away-mode wedge alarm and the continuity deadman; LOCAL, gitignored; absent means auto (macOS Notification Center when available); see docs/wedge-alarm.md
...
state/               runtime records and signals; gitignored
  ...
  <id>.check-trust   private content binding created by fm-check-register.sh for an intentional custom check; with its <id>.check.sh its presence alone keeps supervision required
  ...
  .continuity-deadman.lock .continuity-deadman-alarm .continuity-deadman-last-* .continuity-deadman.log   detached per-home continuity deadman's singleton lock, open-outage episode record, rate-limit stamps, and bounded diagnostic log; the deadman is the one supervision process outside the harness process tree (bin/fm-continuity-deadman.sh; docs/watcher-continuity.md)
  .rewake-pending    written by the Claude Stop auto-arm before a delivered rewake and cleared by bin/fm-wake-drain.sh, so an aged marker means the woken turn never ran; never touch
...
2. **Bootstrap** - detect-only checks (tool/version problems, the worktree-tangle check, harness override, dispatch-profile validation, backlog-backend status) always run, but routine confirmations stay silent by default.
   ...
   Home-local stale Herdr projection cleanup, the continuity-deadman start, and the six bootstrap MUTATING sweeps - non-executing legacy PR-check migration, fleet sync, secondmate convergence, secondmate liveness, pending remote handoff retry, and Relay artifact writes - run only when this session actually holds the lock from step 1; the four network ones among them run in the deferred stage rather than in this section.
...
5. **Fleet-state digest** - after that read-once contract and ahead of the context digest, the compact backlog listing owned by `bin/fm-session-start.sh`; every `state/<id>.meta`; a bounded tail of each task's `state/<id>.status` (labeled as wake-EVENT history, not current state, with the full log path printed for a deeper read); the `state/.afk` flag; and one cheap read of each task's recorded backend endpoint, reporting `alive`, `dead`, or `unknown` - and `unknown` has two distinct causes, named in the line itself: `not checked from here` for a record naming a remote host, which this machine deliberately never probes locally, and `no window recorded`.
...
An endpoint reported `unknown` as `not checked from here` (a remote-host record) is reconciled on its own host, never recovered from here; that is not a dead endpoint and never a local recovery trigger.
...
Relay polling, a registered event source, and a registered custom check each require that same live cycle with no fleet work; `docs/turnend-guard.md`'s "Guard predicates" section owns the full list.
...
- `fleet-dashboard` - load before adding, editing, tagging, linking, or changing the status of a card on the Admiral's Fleet Dashboard (`bin/fm-dashboard.sh`), on every fleet-auditor sweep, on a `check: dashboard-approval <card-id>` wake, which means the Admiral approved that card's plan and the work now needs dispatching, and on a `check: dashboard-note <card-id>` wake, which means the Admiral wrote on that card and firstmate answers on the card in that turn. See `docs/dashboard.md` for the board's architecture and setup.
