...
For a crewmate the always-on watcher does not currently treat a `working:` status line as captain-relevant on its own, so the wake queue alone will not deliver any of these five events while an announced physical action is under way.
BEFORE, START, END, RELEASE, and a silence-break are all absorbed alike whenever they ride a `working:` line, because the brief's status vocabulary has no other nonterminal verb and only an event that coincides with the task's own terminal `done:` or `failed:` line arrives on a wake.
...
That relay guarantee assumes an attended session: under section 8's away-mode stub the sub-supervisor self-handles any signal whose last line is not captain-relevant, with no physical-action exemption and no firstmate turn, so a `working:` announcement from any agent stops there.
...
- `fleet-dashboard` - load before adding, editing, tagging, linking, or changing the status of a card on the Admiral's Fleet Dashboard (`bin/fm-dashboard.sh`), and on every fleet-auditor sweep. See `docs/dashboard.md` for the board's architecture and setup.
