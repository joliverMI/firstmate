...
- `fleet-dashboard` - load before adding, editing, tagging, linking, or changing the status of a card on the Admiral's Fleet Dashboard (`bin/fm-dashboard.sh`), on every fleet-auditor sweep, and on a `check: dashboard-approval <card-id>` wake, which means the Admiral approved that card's plan and the work now needs dispatching. See `docs/dashboard.md` for the board's architecture and setup.
