...
phase_send() {
  ...
  # constant doorbell, routed through the exactly resolved window id this home's
  # meta records - never a foreign same-named window.
  ...
  assert_grep 'send-keys -t @2 -l Firstmate instruction waiting:' "$LOG" "send did not ring the window recorded in this home's meta (exactly resolved)"
