...
#
# Two answers model the parts of real tmux that firstmate's exact-NAME target
# resolution depends on, so a suite cannot pass by a stub's silence:
#   - `new-window -dP -F '#{window_id}'` echoes the created window's id, the
#     rename-safe handle fm_backend_tmux_create_task captures and spawn-time
#     typing then addresses. A stub that printed nothing left that handle empty
#     and let spawn fall back to the name form the id exists to replace.
#   - a `list-windows` asking for '#{window_id} #{window_name}' is answered from
#     the recorded task inventory in state/*.meta, pairing each recorded window
#     NAME with a synthetic @N id that is deliberately NOT that name, so a
#     regression addressing the name where the id belongs cannot pass by
#     coincidence. Every other -F keeps the previous silent success, and
#     FM_FAKE_DUPLICATE_WINDOW still short-circuits ahead of the inventory.
