...
# The fleet-auditor outage: a home whose ONLY duty is a trust-registered custom
# check (bin/fm-check-register.sh) has no state/*.meta record, so before the
# supervision predicate counted registered checks this hook saw nothing to
# supervise and exited 0 at every turn end, leaving the home unwatched.
test_arms_for_registered_check_need_without_inflight() {
  local dir out status
  ...
  write_arm_fixture "$dir" actionable
  out=$(run_autoarm "$dir" 2>/dev/null); status=$?
  expect_code 2 "$status" "a trust-registered custom check must keep the auto-arm active with zero tasks in flight"
