...
The four send primitives use `fm_backend_tmux_exact_target`, the same function `fm_backend_target_exists` answers with, which also accepts an explicitly pane-qualified `session:window.pane` target because a caller-supplied endpoint may legitimately name a pane.
Window removal and the agent-state read use `fm_backend_tmux_exact_target_named` instead - that resolver minus the pane-qualified reading - because both resolve a *recorded* task's own `session:window` metadata field, where the window component is only ever a name: re-reading a dead `sess:fm-1.0` as pane 0 of a live `fm-1` would report an unrelated task's process as this task's `alive`, and would destroy that unrelated task's whole window on removal, since `kill-window` on a pane id removes the pane's entire window.
