...
Existence itself is resolved through `fm_backend_tmux_exact_target`, the same resolver every other gated tmux primitive below shares, rather than a second unpinned lookup: tmux resolves a target-session by prefix exactly like it resolves a target-window, so an unpinned `list-windows -t "$session"` let a dead `dead-sess:fm-1` fall through to a live prefix-colliding `dead-sess-2`'s own window inventory and read that sibling's process under the dead session's label, reporting a nonexistent endpoint `alive`.
When the resolver refuses, an exact-pinned inventory read distinguishes a readable session that simply omits the window (`missing`) from a definitive missing-session or unreachable-server response (also `missing`) or any other read failure (`unreadable`).
...
`tests/fm-backend-tmux-smoke.test.sh` asserts, against a real tmux server: the send/kill/agent-state refusals and that a live decoy pane never receives a misdelivered keystroke, message, or destruction; that a dotted window name receives its own text (and, for kill, is itself removed) rather than affecting a sibling; that an exactly resolvable target still delivers or is still removed; that a session-name prefix collision cannot make `fm_backend_agent_state` read a live sibling's process and report a nonexistent session `alive`; and that `fm_backend_tmux_exact_target` itself, not merely one of its callers, refuses a nonexistent target from inside a live tmux client.
