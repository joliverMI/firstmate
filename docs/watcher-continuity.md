...
It is the one deliberate exception to the rule that the harness owns its hooks' process group, and it retires itself when the session-lock pid stops being a live harness process, when a different session takes over the home's lock (a relaunched primary may run in another pane, and only a deadman started from that session can find it), or when the home has had nothing to supervise for `FM_CONTINUITY_DEADMAN_IDLE_EXIT`.
The lock records the session-lock pid the deadman was started under, and `ensure` from a later session retires a holder recorded under a different pid and starts its own at once, so a relaunched primary is covered from its bootstrap rather than after the old deadman's next tick.
...
Opening a new episode additionally requires that firstmate's own pane, when this process can resolve and read it, does not read mid-turn through the shared busy predicate in `bin/fm-supervisor-pane-lib.sh`: the watcher is down by design for the whole of a rewake handling turn, so a long handling turn is not an outage, while the 2026-09-07 shape left an idle pane behind and still alarms.
An episode already open is not closed by a busy pane; only recovery closes it.
That busy predicate is harness-scoped, and `bin/fm-harness.sh` identifies the markerless harnesses by walking the process ancestry, which a detached process no longer has, so `ensure` resolves the harness while it still sits inside the harness's process tree and hands it to the deadman it starts.
...
`tests/fm-continuity-deadman.test.sh` covers the deadman end to end over real processes with no harness installed: the full predicate and each of its negative controls, one durable wake and one alert per episode, the unhandled-rewake trigger behind a live watcher, a single typed injection per backoff window, an unconfirmed submit that is recorded and still spends that window rather than retyping, deferral to a busy pane and to a composer that is not proven empty, a six-minute handling turn at production grace opening no episode while an idle pane after a dead rewake turn still alarms, episode closure on recovery, singleton `ensure` starting a deadman in a different process group from its starter, a real arm starting it outside the arm's own process group and a later arm keeping the same one, a prompt `stop` inside a production-length tick, self-retirement when the session is gone or replaced by another, `ensure` replacing a superseded session's deadman immediately and handing a detached deadman the harness it resolved from its own ancestry, inertness in a linked task worktree, and the drain retiring the delivered-rewake marker.
...
`tests/fm-bootstrap.test.sh` pins that a lock-holding session start leaves a detached deadman running and that a detect-only session start starts none.
...
A pane that reads busy indefinitely, such as a turn wedged on a hung request that never errors, defers the whole episode rather than only the injection, so no durable wake and no alarm are produced while it stays busy; that is the accepted trade-off for not alarming on every long handling turn.
