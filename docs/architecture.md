...
`/updatefirstmate` fast-forwards the running firstmate repo and registered secondmate homes from each checkout's own configured upstream (falling back to `origin/<default>` when none is set) without touching project clones.
