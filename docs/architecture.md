...
The brief has the crewmate name and use it in a single command - `OWNER_REPO=$(... --print-destination) && gh-axi pr create --repo "$OWNER_REPO"` - so a failure to name the destination short-circuits the create call instead of reaching it as an empty flag; `gh-axi` drops an empty `--repo` and falls back to gh's fork-parent default, so a destination computed in some earlier command whose shell state is gone fails open onto the parent.
