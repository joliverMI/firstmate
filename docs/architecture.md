...
The same cross-check runs on a relaunch's recorded worktree, because a relaunch only proceeds once the prior endpoint reads positively agent-free, so that copy may since have been handed to a live task; a relaunch refusal names the two records that claim one path rather than the pool it never called, and a fresh spawn's refusal closes the window it just created while leaving the copy itself alone.
...
It never blocks a worktree a finished task's own teardown has genuinely retired (its record actually removed); `tests/fm-spawn-pool-worktree-collision.test.sh` owns the regression coverage for both the refusal and that legitimate-reuse case.
Against concurrent spawns the check relies on locking rather than on seeing an unpublished record: every fresh spawn takes the per-home `.task-set.lock` before it acquires any backend container or worktree and holds it until its own record is published, and it takes that lock without waiting, so a second concurrent fresh spawn in the same home refuses outright instead of racing.
That lock is per-home, so two spawns in different homes drawing from the same pool are not serialized and neither sees the other's not-yet-published record; the guard claims nothing about that case.
Accepted limitation, deliberate and not a bug: a record alone is enough to refuse - the guard consults no liveness of the holding task and never mutates another task's record - so a pool slot whose stale record still claims it stays refused until that record is cleaned up.
That happens when `fm-teardown.sh` returns the worktree to the pool before removing that task's meta, or bails at one of its exit points that intentionally retain the record.
The trade is intentional: a stuck refusal is recoverable, while silently reusing a copy a stale-but-uncertain record might still describe is not, so anyone who hits a refused spawn on a genuinely free slot should clean up the offending record rather than loosen this guard.
The real fix belongs to the queued follow-up task that both corrects `fm-teardown.sh`'s worktree-return-then-record-removal ordering and works out how to reclaim a confirmed-dead holder's worktree safely, with its own review rather than bolted onto this guard.
Two leads remain unexplored and are the actual remaining places the originally-reported eviction could live, deliberately deferred rather than investigated here: other backends' own worktree-detection pollers (herdr, zellij, cmux), and timing under real disk I/O and load that a synthetic scratch pool cannot reproduce.
Whoever picks those up should know exactly what has already been tried and come back empty, so the same budget is not spent twice: four direct attacks against the real pool provider - a hard cap with a live holder, a hard cap with a dirty holder, a 9-way concurrent race for the last free slot, and a stale/crashed lease - were each driven against both the CI-pinned and the current treehouse release, and the pool refused cleanly every time.
