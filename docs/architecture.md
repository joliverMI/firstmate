...
Direct-PR tasks, which never reach that pin because they skip no-mistakes entirely, derive their destination from the same place the guard verifies against: `remote.origin.url`, parsed by `fm_pr_github_remote_owner_repo` in `bin/fm-pr-lib.sh`, and passed to `gh-axi pr create --repo`.
