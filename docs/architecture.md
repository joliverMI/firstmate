...
That parser accepts every legitimate spelling of a GitHub origin (userinfo, ports, `ssh.github.com:443`, the `git://` scheme, scp-style), because a form it cannot read would block every task on that project; and no diagnostic ever echoes an origin URL verbatim, since a remote can carry a credential and the guard's stderr is copied into crewmate status files and provisioning logs.
