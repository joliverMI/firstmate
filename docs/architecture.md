...
The PR body travels by `--body-file` from a file the crewmate writes first through a quoted heredoc, because an inline body is shell-evaluated - backticks and `$(...)` in ordinary markdown would be executed and their output substituted - and that file is named by the worktree's own git directory rather than a shared temporary path, so concurrent crewmates cannot overwrite each other's body between the write and the create.
