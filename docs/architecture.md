...
`validate_spawn_worktree` also cross-checks the pool's answer against every other task this home still tracks, refusing rather than accepting a worktree another tracked task's own record already claims while that task's own endpoint could still be running.
The claim only holds while the holder is not confirmed gone: the refusal consults the shared recovery-grade liveness primitive (`fm_backend_agent_alive`) for the holding record's own endpoint, and only a literal `dead` answer releases the worktree - `alive`, `unknown`, an unreadable record, a record with no usable endpoint field, a backend with no recovery classifier, and a remote endpoint that is never probed locally all keep refusing, so an undecided probe can never license two tasks onto one copy.
Releasing also clears that dead holder's own `worktree=` claim, in `fm-spawn.sh` and strictly before the new task publishes its own record naming the copy: two records claiming one copy is the original hazard with a delay fuse, since the next ordinary cleanup of the dead holder would return the copy the new live task is working in, while a crash between the clear and the publish leaves zero claimants, which a later spawn or an operator can reconcile.
Only the confirmed-dead gate reaches that clearing, so an ambiguous answer can never erase a live worker's own record, and a claim that cannot be cleared refuses the spawn rather than proceeding.
Known dependency: `fm_backend_agent_state`/`fm_backend_agent_alive` is being corrected by a separate, concurrent task because it has reported some dead endpoints as alive; this guard is correct under both the current and the corrected behavior, because a false `alive` only ever costs an extra refusal, never a wrongful reuse or a wrongful clear.
...
It never blocks a worktree a finished task's own teardown has genuinely retired, nor one whose holder's endpoint is confirmed gone; `tests/fm-spawn-pool-worktree-collision.test.sh` owns the regression coverage for the refusal, for the ambiguous and unprobeable answers that must keep refusing, and for both legitimate-reuse cases.
One case still refuses a genuinely free slot: `fm-teardown.sh` returns the worktree to the pool before it removes the task's own record, so a teardown that stalls in between - the herdr endpoint-confirmation-ambiguous path is the clearest one, since it deliberately retains every durable task record - leaves a claim whose endpoint reads ambiguous rather than dead.
Clearing `worktree=` at the moment teardown actually returns the worktree is the root fix for that window and is filed as its own separate task rather than folded into this guard.
