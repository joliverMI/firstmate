...
Generated secondmate charters reject generic receipt or start acknowledgements for ordinary routed work, key only supervisor-actionable material phase reports, and close an opened phase with a same-key later state or `resolved` event, while the structured home remains authoritative even if that closure is missing.
The one carve-out is `AGENTS.md` section 9's physical-action announcement contract, which the charter appends as its own status lines despite that default.
...
A pull-based guard (`bin/fm-guard.sh`) warns through supervision tool output if the primary checkout is tangled, if a home that needs supervision has an unhealthy model-aware supervision verdict, or if queued wakes are waiting to be drained.
...
On every verified primary harness, tracked hook integration gives the primary session a push-based backstop: when the home needs supervision and no identity-matched watcher lock with a fresh beacon is live, blocking-capable Stop hooks block and nonblocking turn-end integrations force one bounded follow-up.
...
The daemon escalates captain-relevant events, plus a bounded recheck for a declared pause or a verified captain-held transfer that remains idle, naming which human that wait is on, as one batched, single-line digest using the canonical `away-supervisor` kind from `bin/fm-operational-input.sh` so firstmate can distinguish it structurally from real messages.
...
`fm-spawn.sh` also owns the base-freshness boundary for every fresh ship and scout: no worker starts until its clean task worktree matches the fetched tip of the development remote's resolved default branch (the branch's configured upstream, falling back to origin), and any unsafe or unverifiable base stops the spawn.
Its header owns the exact refusal mechanics, while `tests/fm-spawn-pool-base-freshen.test.sh` owns the portable regression coverage.
`validate_spawn_worktree` also cross-checks the resolved copy - the pool's answer on a fresh spawn, the task's own recorded worktree on a relaunch - against every other task this home still tracks, and refuses rather than accepting a path another tracked task's record already claims.
A relaunch needs the same check because it proceeds as soon as the prior endpoint reads positively agent-free, so a dead task's recorded copy may since have been handed to a live task.
This is a hardening guard, not a fix for a reproduced failure: the pool provider was never observed handing back a worktree still in use, so the reported eviction stays unconfirmed at that layer, and what this closes is only the acceptance side, where `fm-spawn.sh` previously trusted the reported worktree and would have let this task's freshen step or eventual teardown destroy another task's unlanded work.
Within one home, concurrent spawns are serialized by the per-home `.task-set.lock` that every fresh spawn takes without waiting before it acquires any backend container or worktree and holds until its own record is published.
Two spawns in different homes drawing from the same pool are not serialized by that per-home lock and neither can see the other's unpublished record, and the guard claims nothing about that case.
A record alone is enough to refuse: no liveness of the holding task is consulted and no other task's record is ever mutated, so a slot whose stale record still claims it stays refused until that record is cleaned up.
That trade is deliberate, because a stuck refusal is recoverable and silently sharing one copy between two tasks is not, so a refusal on a genuinely free slot is fixed by retiring the offending record rather than by loosening the guard.
Such a stale claim comes from `fm-teardown.sh`, which returns a worktree to the pool well before it removes that task's meta and deliberately retains the record at several of its bail-out points; correcting that ordering, reclaiming a confirmed-dead holder's copy, and the two unexplored leads for the original report (other backends' own worktree-detection pollers, and timing under real disk load that a synthetic pool cannot reproduce) are each deferred to their own work rather than bolted onto this guard.
It also comes from a terminal restart, because the pool treats a copy as free once no process is left inside it: stopping and reprovisioning a session kills every task pane's shell at once, so every tracked task's copy becomes reusable while every record survives, and the first spawn afterwards is handed one of those copies and refuses until the record naming it is retired.
`tests/fm-backend-herdr-presentation-e2e.test.sh` restarts a real session repeatedly while keeping its own fixtures tracked, so it reinstates one holder process per still-recorded copy after each restart to keep the pool's view consistent with the records it maintains.
The same header owns this refusal's exact mechanics and its source-specific recovery hints, while `tests/fm-spawn-pool-worktree-collision.test.sh` owns the regression coverage for both the refusal and for the legitimate reuse of a copy a finished task's teardown genuinely retired.
...
Verification is a destination check, not an existence check: after pinning, it reads the destination back with `gh repo set-default --view` and requires that value to name the project checkout's own origin owner/repository (case-insensitively, as GitHub itself compares them).
A pin that merely exists proves nothing - an origin repointed at the parent pins `gh` just as successfully to the wrong repository, and both this guard and `fm-pr-check.sh`'s backstop would then agree on it.
The gate is checked against the *project checkout's* origin rather than its own, so a gate whose origin has drifted to the fork parent cannot verify happily against itself.
That verified destination, not the write, is the invariant it enforces: an already-correct destination is confirmed with a local config read plus the offline `--view` and left alone, and `gh repo set-default origin` runs only when the destination is missing or wrong.
This matters because the guard runs in the Setup step of every no-mistakes task, from concurrent crewmate worktrees that share one checkout's common git config - an unconditional authenticated write there would turn an API blip or a config-lock collision into a blocked task whose destination was never at risk.
Every refusal carries `gh`'s own stderr, since that diagnostic is what a blocked crewmate records as `blocked:` and what an operator recovers from.
It fails loudly - non-zero exit, a diagnostic naming exactly what could not be pinned or verified - rather than proceeding when origin is missing or unparseable, the gate cannot be discovered, a pin does not read back after being written, or the destination read back fails, is empty, or names any other repository; a non-GitHub origin is a deliberate no-op, since this specific default is gh-specific.
It is invoked everywhere `no-mistakes init` already is - project initialization (the `project-management` skill), secondmate home seeding (`fm-home-seed.sh`, `fm-remote-home-provision.sh`), and the contributor gate setup in [`CONTRIBUTING.md`](../CONTRIBUTING.md) - and unconditionally in every no-mistakes-mode ship brief's Setup step, so it runs before implementation work and again before every validation run, not only once at first setup.
Remote provisioning (`fm-remote-home-provision.sh`) is the one caller that warns loudly instead of aborting when the pin cannot be applied: `gh` is an optional tool on a remote host and provisioning itself never opens a pull request, so the refusal is deferred rather than skipped - the same guard runs again in that host's next no-mistakes ship brief and blocks the task there until the destination is pinned.
Direct-PR tasks, which never reach that pin because they skip no-mistakes entirely, get the same determination through the guard's `--print-destination` mode: a local `remote.origin.url` read parsed by `fm_pr_github_remote_owner_repo`, printing `owner/repo` and nothing else, with no gh call, no network, and no dependence on the pin.
The brief has the crewmate capture and consume that value in a single command, so the destination is named and used in one shell and a destination that could not be determined stops the create call rather than reaching it as an empty flag: `gh-axi` drops an empty `--repo` and falls back to gh's fork-parent default, so a destination computed in some earlier command whose shell state is gone fails open onto the parent.
The PR title and body both travel through files the crewmate writes first with quoted heredocs, never as inline arguments, because author-written text is not safe in a shell word: backticks and `$(...)` in ordinary markdown would be executed and their output substituted into the body, and a single apostrophe in a title would end the quoted string and kill the whole line before any of it ran.
Those files are named by the worktree's own git directory rather than a shared temporary path, so concurrent crewmates cannot overwrite each other's title or body between the write and the create.
`bin/fm-brief.sh`'s generated direct-PR instructions own the exact command; this section describes only what it must guarantee, so the two cannot drift.
No `gh` command decides it, because two separate `gh` resolutions point the wrong way for a fork - `gh pr create` defaults to the parent, and `gh repo view` with no repository argument picks a remote by gh's own preference order (`upstream` before `origin`), so on a checkout carrying an `upstream` remote it names that remote's repository rather than the checkout's own.
That parser accepts every legitimate spelling of a GitHub origin (userinfo, ports, `ssh.github.com:443`, the `git://` scheme, scp-style, and `github.com-<alias>` SSH config Host aliases), because a form it cannot read would block every task on that project; and no diagnostic ever echoes an origin URL verbatim, since a remote can carry a credential and the guard's stderr is copied into crewmate status files and provisioning logs.
It is also the single authority on whether an origin is GitHub at all: the parse runs first, and a URL it cannot name refuses only when its *host* is a GitHub host, so a project on another forge is out of scope rather than blocked - the fork-parent default is a gh behaviour on GitHub forks and cannot reach one.
`--print-destination` reports that distinction as exit 3, and the direct-PR brief uses it to open the pull request unprotected where there is nothing to protect against, while still refusing to create anything when the destination is undetermined on a repository where the hazard is real.

Two boundaries here are accepted limitations rather than defects, and both are stated so the next reader does not mistake either for one.
First, only `github.com`, `ssh.github.com`, and their single-label `-<alias>` SSH-config Host forms are recognized as GitHub. A GitHub Enterprise Server host carries the same fork-parent default but is not recognized, and an origin on any unrecognized host is skipped rather than pinned - so the guard announces that skip out loud in both modes, naming the host it did not recognize, and the first project on such a host reports the gap itself instead of quietly proceeding unpinned.
The alias suffix must be one dotless label because saying yes aims a pull request at github.com: `github.com-mirror.example.net` is a different domain that merely starts the same way, and naming its `owner/repo` would target an unrelated repository on github.com itself.
Second, an SSH-config Host-alias origin is named successfully by `--print-destination`, which is a text parse, but is refused by the pin-and-verify path, which is not: `gh` cannot resolve an alias host at all (`none of the git remotes configured for this repository point to a known GitHub host`), so no-mistakes mode blocks on such a project. Teaching the guard to resolve local SSH aliases would be new capability for a pattern nothing in this fleet uses.
...
Local-only projects, clones missing the remote the refresh would fetch from, and fetch failures remain benign skips.
The refresh also prunes local branches whose remote is gone and that no worktree still needs.

## Self-updates stay safe

`/updatefirstmate` fast-forwards the running firstmate repo and registered secondmate homes from each checkout's own configured upstream (falling back to `origin/<default>` when none is set), then re-reads updated instructions and nudges updated secondmates without touching project clones.
For a remote route, the configured code root updates from its own configured upstream on that host before the persistent home fast-forwards to the code-root commit.
