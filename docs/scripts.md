...
| `fm-status-board.sh`     | Render the bearings classification and fleet snapshot detail into the captain's five-section web status board |
| `fm-dashboard.sh`        | The only agent-facing command surface for the Admiral's Fleet Dashboard task board and its server process ([dashboard.md](dashboard.md)) |
| `fm-fleet-audit-tick.sh` | The fleet auditor's host-cron timer: heartbeats every invocation, sweeps when the dashboard's own interval setting says it is due ([dashboard.md](dashboard.md#the-timer)) |
| `fm-fleet-audit-sweep.sh` | One fleet-auditor sweep - claims the dashboard's sweep lock, runs the mechanical checks, records the result; used by both the timer and the Force Audit button |
...
| `fm-claude-trust.sh`     | Pre-register Claude Code's workspace trust for a fresh task worktree before a claude spawn launches into it |
...
| `fm-procevent-river.sh`  | Long-poll the River voice service as a standing source, batching each takeover burst into one wake |
...
| `fm-watch.sh`            | Singleton-safe always-on watcher: absorb benign wakes, detect stalled local-secondmate wake queues, queue and exit on actionable ones |
| `fm-continuity-deadman.sh` | Detached per-home singleton that records, alerts on, and self-recovers from a supervision chain that stopped outside the turn boundary |
...
| `fm-supervisor-pane-lib.sh` | Decide whether firstmate's own pane is safe to type into right now                   |
| `fm-wedge-alarm-lib.sh`  | Own the configurable, pane-independent active alert channels and their bounded dispatch |
...
| `fm-ff-lib.sh`           | Shared guarded fast-forward helper for configured-upstream pulls (falling back to origin) and local secondmate syncs |
| `fm-dev-remote-lib.sh`   | Single owner of development-remote resolution: which remote and branch a checkout's default branch actually updates from (its configured upstream, e.g. a fork), falling back to `origin/<default>` and saying so |
...
| `fm-pr-destination-guard.sh` | Pin and verify gh's pull-request destination for a project checkout and its no-mistakes gate, or print that destination for a caller that must name it |
