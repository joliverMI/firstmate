...
- **The streak's dominant, root-caused failures are this suite**, not a new incident: CI logs for the prior red streak on this fork's `main` (jobs `95243266311`, `95990618501`, `95755272024`, spanning 2026-08-16 through 2026-08-19) show `not ok` lines against `Pi must deliver the actionable wake after bounded hung-successor recovery`, `OpenCode watch plugin must arm only when this session owns the fleet lock`, and `OpenCode watch plugin must not treat external healthy output as an owned arm`, all in `tests/fm-pi-watch-extension.test.sh`. The first two are exactly the cause-B and cause-A assertions the table above attributes and PR #14 fixes. The third is in the same file but is *not* one of the two attributed causes in that table; it is recorded here as an observed failure in the streak, not as something the cause table accounts for. `git diff --stat b98e098..e2786ac -- .opencode .pi tests/fm-pi-watch-extension.test.sh` is empty: nothing has touched the arm/watch code or this suite since the fix merged.
- **Two failures in the same streak are outside this suite and are not attributed to PR #14's fix.** Run [`32150616824`](https://github.com/joliverMI/firstmate/actions/runs/32150616824) (2026-08-18, `main` at `3c8f796`) also logged `not ok - next bounded scan did not resume with the following child` in `tests/fm-inactive-reconcile.test.sh` (job `95755272021`, `Behavior portable serial 4`) and `not ok - spawn with --card should succeed` in `tests/fm-dashboard-card-link.test.sh` (job `95755272024`, `Behavior portable serial 3`). Neither is root-caused here.
  `tests/fm-inactive-reconcile.test.sh` is untouched since `2d550fe` (#2167), long before this streak, and shares the `watcher-wake-lock` family (`bin/fm-test-run.sh`) with the arm-readiness suite, so shared CI-runner load contention during that one run is the plausible but **unproven** explanation.
  `tests/fm-dashboard-card-link.test.sh` was added by the very commit whose run failed (`3c8f796`, #9) and was touched again by #11 and #16, so this reads as a one-off in a brand-new test rather than a recurring defect - also not independently root-caused.
  Neither has failed in any run since: no `main` run from `32256268913` (2026-08-19T13:06Z) onward has failed (`32441040308` was superseded and cancelled; the rest are green through `32443727353`), spanning several further merges. That is evidence of absence, not proof of a fix.
...
- **Fresh local reproduction, same suite:** 10/10 passed on an idle host; 9/10 passed under added load (40 busy-loop processes layered on top of this shared host's own pre-existing concurrent load, 1-minute load average already 9.5-18.7 before the addition - a heavier and less controlled condition than the clean 5x-oversubscription phase above, not a like-for-like re-measurement of it). The one loaded failure was `OpenCode watch plugin must use FM_HOME state outside the repo root` (`test_opencode_primary_watch_plugin_uses_effective_state_home`) - an assertion this record's cause table does not cover, and a different one than the baseline-control section above observed on this same host. Consistent with that section's own point that this suite's load-sensitivity varies by host and run, this reproduces the suite's known bounded-poll-under-extreme-contention residual rather than a regression of the fix above: the fix is unmodified, the assertion is outside its scope, and it did not reproduce on 9 of 10 loaded runs or on any idle run. Not remediated here; flagged as a candidate for a follow-up hardening pass on that one case.
- **Conclusion: `main` is green.** The 11-hour red-CI report describes an incident whose dominant cause this suite's fix already resolved before this check ran, with two unattributed non-suite failures that have stayed quiet since; it is not a description of `main`'s current state.
