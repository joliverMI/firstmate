...
- **The streak's dominant, root-caused failures are this suite**, not a new incident. The prior red streak on this fork's `main` is seven failed runs between 2026-08-16T23:28Z and 2026-08-19T07:24Z, containing eight failing serial jobs; the logs of all eight were read, and this is the complete tally rather than a sample.
  ...
  Two details cut against the report's own framing and are worth keeping. `95726593907` is `Behavior portable serial 1`, so the streak was not confined to shards 3 and 4. And `Behavior portable serial 3` in run `32037964172` was cancelled rather than failed, logging no `not ok` at all, so it contributes no assertion in either direction - a second incomplete shard, this one inside the streak.
