...
FM_WATCH_ARM_NO_LOGIN_SHELL=1   # test-only: spawn the Pi/OpenCode arm child under `bash -c` instead of `bash -lc`, skipping /etc/profile and the account's profile files. Leave unset in production so fm-watch-arm.sh inherits PATH additions a profile provides (e.g. an nvm-managed node)
