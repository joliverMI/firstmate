...
On tmux, the daemon accepts a target only when it resolves to exactly one live endpoint, so a name tmux would otherwise prefix-match, or a bare window name live in more than one session, is refused at startup instead of supervising the wrong pane; [`tmux-backend.md`](tmux-backend.md) owns that resolution contract.
...
A standalone-clone home cannot receive a primary-local commit through that no-fetch sync, so it receives the rule through `/updatefirstmate`'s fetch-based refresh instead.
...
`bin/fm-procevent.sh deliver <source-id> <sequence> -- <command>...` closes the gap a manually-remembered `handled` call leaves open for a handler whose result-handling itself runs one downstream command: it checks handled status, runs the command, and marks the generation handled as one call under the source lock, so a generation already delivered runs the command zero more times no matter how many times a repeat wake for it is handled, and a nonzero exit leaves the generation unhandled for the ordinary retry instead of being marked delivered.
...
FM_GUARD_GRACE=300      # seconds before guard warnings, arm health checks, the primary turn-end guard, and the continuity deadman treat a watcher beacon as stale
...
FM_PAUSE_RESURFACE_SECS=3600       # seconds before the watcher re-surfaces a declared external wait or verified captain-held transfer for a recheck, including a live busy pane past FM_BUSY_TURN_MAX_SECS; the away-mode daemon uses the same setting for declared external waits
