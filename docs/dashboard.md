...
- **`bin/fm-backlog-handoff.sh --card <card-id>`** (routing to a secondmate): the same best-effort link, reached before any task exists to hold `dashboard_card=`. Once the single named item has genuinely landed in the secondmate's own backlog (locally, or confirmed delivered to a remote one), it best-effort sets the card's `backlog_ref` to `<secondmate-id>:<item-key>` and `agent` to the secondmate id, then advances a `not_started` card to `working` - keyed by secondmate identity rather than a task id, since none exists yet at handoff time. The pairing is held in the handing-off home's own state directory (`state/handoff-cards/<secondmate-id>`, written once staging has landed), so it survives a crash between staging and delivery and is what `--resume-pending` reads back. A pair is retired only once the board has genuinely answered for that pair - the link confirmed, or a more precise claim found already in place - never on the strength of having merely attempted it. Any pair whose link fails stays recorded for the next arrival at this secondmate to retry, so a failed attempt can never silently orphan the card. That deliberately includes a card the host says it does not have: an HTTP 404 proves only that *some* host answered, not that the answering host was the board, and a stale `config/dashboard-url` pointing at a machine that still serves HTTP but no longer runs the dashboard would 404 every card alike - honouring that as a verdict would discard every pending link at once, which is precisely the loss this record exists to prevent. What is bounded for an unlinkable card is the *noise*, not the record: each such pair gets one stderr warning and one `audit-log --fleet` entry, remembered in the `state/handoff-card-warnings/<secondmate-id>` ledger, rather than a fresh identical pair on every arrival - an unbounded stream of identical findings would bury the log it is recorded in. That ledger is advisory only; losing it costs a duplicate warning, never a link. The handed-off item itself is never rewritten: which card a deliverable serves is firstmate-local bookkeeping, and putting it in the item's body would mean the handoff performing a read-modify-write on backlog content it does not own. Reaching `testing` still depends on the secondmate itself later passing that same `--card` id to its own `fm-spawn.sh`/`fm-teardown.sh` cycle against this same shared board (see "Reaching the board from a secondmate" below) - the same reliance every other `--card` use already has on the dispatching agent remembering the flag, now also covering the handoff case rather than a new, weaker guarantee.
...
Re-recording the same item key with a *different* card id keeps both pairs and says so loudly on stderr, rather than replacing the old one. An unresolved pair may already carry a half-written link on its own card, and dropping it would leave that card dangling with nothing left to finish it - the same silent orphaning the record exists to prevent. Keeping it is not the same as serving it, though: the displaced pair is marked superseded in `state/handoff-card-superseded/<secondmate-id>` and is never written to again, so a card the operator has already disowned is never marked as `working` under an agent that is not serving it. It stays in the record, reported once, purely as the reminder that it may still carry a half-written link somebody has to unpick by hand. The mark is reversible from the CLI alone: naming that same card again for that item clears it, so a `--card` typo in the *correction* is recoverable without hand-editing state.
