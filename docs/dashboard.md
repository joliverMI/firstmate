...
A fleet audit once traced every status discrepancy it had ever raised to one root cause: no card carried `agent` or `backlog_ref`, so nothing connected a worker to the card it served. A status transition then only happened if someone remembered to make it by hand, and the transition needed most - the move to `review` - fell due at exactly the moment the worker doing the remembering was being torn down. Cards with a live worker got maintained beautifully; cards whose worker had finished froze at whatever they last said.

The fix establishes the link mechanically at the three points that already own where a piece of work is dispatched, routed, and finished, reusing the fields the board already had rather than adding a parallel path. The identity write and not-started-to-working advance shared by `fm-spawn.sh` and `fm-backlog-handoff.sh`, and the advance-on-landing shared by `fm-teardown.sh`, are each a single call into `bin/fm-dashboard-link-lib.sh` rather than three independent copies, so a change to either transition rule has one owner:
...
**A note he writes wakes firstmate the same way.**
A note is the other board write that hands work back to the fleet: when he posts on a card from the board, on any of its note tabs, or any caller adds a note with `author` set to `admiral`, the note write publishes one durable `check: dashboard-note <card-id> ...` record through the same writer, after the note commits.
A note by an agent or by firstmate publishes nothing - only his note is an ask that somebody owes an answer.
The failure rule is the approval's, unchanged: a wake that cannot be published never costs him the note, which is saved either way, and the failure is written to the server log and to the discrepancy log (`note-wake-unpublished`) where it reads as a card he wrote on with nobody told.
A note nobody is woken about goes unanswered by construction just as an unwoken approval rots: his question on a card once sat for hours because only an approval woke anyone, which is why this is code rather than an instruction to check the tab.
...
### The one rule behind six separate bugs

Six different writes in this change each ended with him no longer being asked something he had never answered, and they were found one at a time: an approval writing a same-status history row that reset his clock; an approval quieting a card it had never been asked about; a re-ask restarting the clock on a card he was already blocked on; a plan edit leaving his reply to the old plan standing as an answer to the new one; a plan created on a card that renders no approval box; and the landing advance moving a card out of the only status he can approve from.
They are one family and one rule, and it is worth stating once rather than leaving six fixes to be inferred from:

**An outstanding ask is his until he answers it: no write but his own may make it look answered, or leave it somewhere he cannot answer it.**

The first four make an outstanding ask LOOK ANSWERED.
The auditor's age check is the only thing that catches a card he has been left waiting on, and it reads timestamps: three of those bugs froze his clock so a card he was still waiting on looked freshly attended to, and the fourth let a stale answer stand so a card whose question had changed looked already answered.
The test the code has to be able to answer is whether he has seen and answered THIS ask, not whether he has ever replied to this card, which is why a re-ask and a plan edit both date the newest ask while neither touches the timestamp his age is measured from.

The last two make an outstanding ask UNANSWERABLE.
A plan written onto a card that renders no approval box is a recommendation with no way to accept it, and a `needs_review` card advanced into `review` because its work landed takes the question out of the one status where the button exists and out of the one check that would ever raise it again.
Both ends are the same failure: he stops being asked, without anyone ever deciding that he should.
Whoever adds the next write against a card should check it against that sentence before assuming it is inert, and a write that fires mechanically with no human in the loop - a teardown, a sweep, a resume - must resolve any uncertainty toward leaving the ask standing, because a card left on his board is seen while an ask quietly retired is not.
